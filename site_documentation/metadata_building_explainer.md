...
## Reference: The Data Page (`/data.html`)

The master CSV produced by step 5 above isn't just consumed internally — it's also served directly to site visitors. The Data page's "Download Data" modal offers it as a verbatim, byte-for-byte "Complete Metadata" CSV (copied at build time by `_plugins/copy_source_csv.rb`, not regenerated through Liquid, so it always matches `_data/cb_complete_metadata_images_tropes_reprints_transcripts.csv` exactly). A simplified "Article Metadata" CSV (one row per article, core publication fields only) and a "Metadata Facets" JSON (unique value counts) are also generated from it for visitors who don't need the full pipeline output. See `site_documentation/data_relationships_erd_explainer.md` for why the master CSV has 4,428 rows rather than 481.
